...
static void init_clock_stm32f3_16mhz_hse(void);
...
#elif defined(BOARD_CONFIG_MCU_STM32F3) && defined(BOARD_CONFIG_OSC_HSE_16MHZ)
    init_clock_stm32f3_16mhz_hse();
...
static void init_clock_stm32f3_16mhz_hse(void)
{
    ...
    rcc_osc_off(RCC_PLL);
    rcc_wait_for_osc_not_ready(RCC_PLL);
    rcc_set_prediv(RCC_CFGR2_PREDIV_DIV2); // 16 Mhz
    rcc_set_pll_source(RCC_CFGR_PLLSRC_HSE_PREDIV);
    rcc_set_pll_multiplier(RCC_CFGR_PLLMUL_PLL_IN_CLK_X9); // 72 MHz
